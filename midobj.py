...
#   lower-cell white arrow extraction -> geometry classification.
#
...
# Do not accept weak STRAIGHT reads. False A/B straight mistakes were low-confidence (~0.82). Real straight arrows
# in samples score >0.90, so weak STRAIGHT reads are ignored.
...
def _arrow_mask_from_cell(cell):
    ch, cw = cell.shape[:2]

    y0 = int(ch * ARROW_Y0_RATIO)
    y1 = int(ch * ARROW_Y1_RATIO)
...
def _select_arrow_component(mask):
...
def read_arrow_direction(cell, debug=False):
    mask, roi, offset = _arrow_mask_from_cell(cell)
    if mask is None:
        return None, 0.0

    arrow = _select_arrow_component(mask)
    if arrow is None:
        return None, 0.0

    direction, confidence = _classify_arrow_mask(arrow)
...
def _board_target_score(board, frame_shape):
...
def select_target_board(board_reads, frame_shape):
    ...
    for board in board_reads:
        score = _board_target_score(board, frame_shape)
        if score > best_score:
            best_score = score
            best = board
...
class ObjectRecognizer(Node):
    ...
    def _check_parameter_goal(self):
        ...
        changes. The current goal persists after board exit unless ServerCommunication or
        parameter explicitly changes it.
        """
